...
namespace LpSolveDotNet.Demo.Idiomatic
{
    // Putting this using directive *inside* namespace so LpSolve classes are not referenced from LpSolveDotNet namespace
    // over LpSolveDotNet.Idiomatic because of current class's namespace
    using LpSolveDotNet.Idiomatic;
    ...
    internal class OriginalSample
    {
        ...
        private /* unsafe */ static void logfunc(LpSolve lp, string Buf)
        {
            Debug.Write(Buf);
        }
